...
export class EventController {
  constructor(container, eventData, onDataChange, onChangeView) {
    ...
    this._newEventData = cloneDeep(this._eventData);
  ...
  init() {
    ...
    this._eventEdit.getElement()
      ...
      .addEventListener(`submit`, (evt) => {
        ...
        this._newEventData.timeStart = formData.get(`event-start-time`) ? new Date(this._eventEdit.getElement().querySelector(`#event-start-time-1`).getAttribute(`datetime`)) : ``;
        this._newEventData.timeEnd = formData.get(`event-end-time`) ? new Date(this._eventEdit.getElement().querySelector(`#event-end-time-1`).getAttribute(`datetime`)) : ``;
        this._newEventData.price = formData.get(`event-price`);
        this._newEventData.isFavorite = this._eventEdit.getElement().querySelector(`#event-favorite-1`).checked;
